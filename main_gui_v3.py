...
try:
    ...
    from imu_csv_export_v3 import export_csv_smart_v3, analyse_topic_fitness
except ModuleNotFoundError:
...
class MainWindow(QMainWindow):
    ...
    def _build_menu(self) -> None:
        ...
        self.act_export.triggered.connect(
            lambda: export_csv_smart_v3(self, gps_df=self._gps_df))
        ...
        act_fit = QAction("Export Readiness / Rotation Check …", self)
        act_fit.setEnabled(False)
        act_fit.triggered.connect(self._show_fitness_dialog)
    ...
    # ------------------------------------------------------------------ Fitness-Analyse
    def _show_fitness_dialog(self) -> None:
        if not self.dfs:
            return
        info = analyse_topic_fitness(self.dfs, self._gps_df)
        txt = ["<b>Export Readiness</b><br><table>"]
        txt.append("<tr><th>Topic</th><th>Labeled?</th><th>Rotation</th></tr>")
        for row in info:
            col1 = row["topic"]
            col2 = ("<span style='color:green'>➕</span>"
                    if row["has_label"] else
                    "<span style='color:grey'>➖</span>")
            col3 = ("<span style='color:green'>ok</span>"
                    if row["rot_ok"] else
                    "<span style='color:orange'>⚠️</span>")
            txt.append(f"<tr><td>{col1}</td><td align=center>{col2}</td>"
                       f"<td align=center>{col3}</td></tr>")
        txt.append("</table>")
        QMessageBox.information(self, "Export Status", "".join(txt))
