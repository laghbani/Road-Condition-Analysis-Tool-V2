...
# comfort class thresholds (ISO 2631-1)
COMFORT_COLORS = [
    (1.72, "#00ff00"),      # Comfortable
    (2.12, "#ffff00"),      # Slightly uncomfortable
    (2.54, "#ffa500"),      # Uncomfortable
    (3.19, "#ff0000"),      # Very uncomfortable
    (float("inf"), "#ee00ff"),  # Extremely uncomfortable
]


def col_for(value: float) -> str:
    """Return color for *value* according to ISO comfort classes."""
    for thr, col in COMFORT_COLORS:
        if value < thr:
            return col
    return COMFORT_COLORS[-1][1]
...
def calc_awv(
    ...
    peaks, _ = find_peaks(awv, height=peak_thr, distance=int(fs * min_dist_sec))
